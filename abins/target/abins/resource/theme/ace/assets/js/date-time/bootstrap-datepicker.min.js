!function(d) {
	...
	var a = function(h, g) {
		...
		this.picker = d(c.template).appendTo(this.isInline
				? this.element
				: "body").on({
	...
	a.prototype = {
		...
		fill : function() {
					...
					if (this.calendarWeeks) {
						var h = new Date(+B
								...
								* 86400000), k = new Date(+h
								+ (7 + 4 - h.getUTCDay()) % 7 * 86400000), j = new Date(+(j = f(
								k.getUTCFullYear(), 0, 1))
								+ (7 + 4 - j.getUTCDay()) % 7 * 86400000), q = (k - j)
								/ 86400000 / 7 + 1;
			...
			this.picker.find(".datepicker-days tbody").empty().append(o
					.join(""));
		...
		showMode : function(g) {
			...
			this.picker.find(">div").hide().filter(".datepicker-"
					+ c.modes[this.viewMode].clsName).css("display", "block");
			this.updateNavArrows()
		}
	};
						...
						switch (g) {
							case "MM" :
								p = d(e[n].months).filter(function() {
									var i = this.slice(0, m[o].length), s = m[o]
											.slice(0, i.length);
									return i == s
								});
